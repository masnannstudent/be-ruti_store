...
func (s *HomeService) GetAllOrders(page, pageSize int) ([]*entities.OrderModels, int64, error) {
	result, err := s.repo.GetAllOrders(page, pageSize)
	if err != nil {
		return nil, 0, err
	}

	totalItems, err := s.repo.GetTotalOrderItems()
	if err != nil {
		return nil, 0, err
	}
...
func (s *HomeService) GetOrdersPage(currentPage, pageSize int) (int, int, int, int, error) {
	totalItems, err := s.repo.GetTotalOrderItems()
	if err != nil {
		return 0, 0, 0, 0, err
	}
